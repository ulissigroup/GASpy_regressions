## hacked code of pymatgen Wulff module
...
class WulffShape(object):
    ...
    def get_plot(self, color_set='Spectral', grid_off=True, axis_off=True,
                 show_area=False, alpha=1, off_color='red', direction=None,
                 bar_pos=(0.75, 0.15, 0.05, 0.65), bar_on=False,
                 legend_on=True, aspect_ratio=(8, 8), color_data=None, hatched_facet_list=[(9,9,9)], hatch_pattern = '/'):
        """
        ...
        if color_data is None:
            color_data = self._get_colors(color_set,alpha,off_color)
        color_list = color_data[0]
        color_proxy = color_data[1]
        color_proxy_on_wulff = color_data[2]
        miller_on_wulff = color_data[3]
        e_surf_on_wulff = color_data[4]
        ...
        for plane in self.facets:
            ...
            if self.miller_list[plane.index] in hatched_facet_list:
                tri.set_hatch(hatch_pattern)
