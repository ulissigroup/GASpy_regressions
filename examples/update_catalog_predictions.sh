...
if ! squeue -u ktran | grep -E 'regress|predict|push'; then
    ...
    push=$(sbatch --parsable --dependency=afterany:$predict_CO,afterany:$predict_H,afterany:$predict_N,afterany:$predict_O,afterany:$predict_OH,afterany:$predict_OOH /global/project/projectdirs/m2755/GASpy_workspaces/GASpy/GASpy_regressions/examples/regression_scripts/push_predictions.sh)
fi
